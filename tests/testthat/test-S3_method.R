...
test_that("Print method handles invalid and non-standard savvySh_model objects correctly", {
  ...
  expect_true(any(grepl("V1", output_no_intercept)))  # Checks if `V1` is included
  expect_true(any(grepl("V2", output_no_intercept)))  # Checks if `V2` is included
  expect_true(any(grepl("V3", output_no_intercept)))  # Checks if `V3` is included
  expect_false(any(grepl("Intercept", output_no_intercept)))  # Intercept should not be present
})
...
test_that("Predict method works correctly for savvySh_model", {
  ...
  expect_equal(length(coef_no_intercept), ncol(x))  # No intercept
...
test_that("Predict method handles missing or improperly structured coefficients", {
  ...
  fit_invalid$coefficients <- 1:5  # Not a list
  expect_error(
    predict(fit_invalid, newx = x[1:5, ], type = "response", estimator = "St"),
    "Invalid 'savvySh_model' object: coefficients are missing or improperly structured."
  )
  ...
  expect_warning(
    predict(fit, newx = x[1:5, ], type = "response"),
    "No estimator specified. Defaulting to the first available estimator:"
  )
  expect_type(predictions_default, "double")
  expect_equal(length(predictions_default), 5)
  expect_error(
    predict(fit, newx = x[1:5, ], type = "response", estimator = "InvalidEstimator"),
    "The specified estimator 'InvalidEstimator' is not available. Choose from:"
  )
...
test_that("Predict method handles errors for 'response' type", {
  ...
  expect_error(
    ...
    "The number of variables in 'newx' must match the number of predictors in the model."
  )
  expect_error(
